...
try {
    ...
    * */
        ...
        if ($ussd_string_exploded[1] == "1")
        {
            ...
            switch($count)
            {
                ...
                case 3:
                    ...
                    {
                        save("MDSP2000328574", "Thank you for subscribing to daily video service", $dbh, $phone);
                    }
                    else if ($ussd_string_exploded[2] == "2")
                    {
                        save("MDSP2000315605", "Thank you for subscribing to weekly video service", $dbh, $phone);
                    }
                    else if($ussd_string_exploded[2] == "3")
                    {
                        save("MDSP2000315607", "Thank you for subscribing to monthly video service", $dbh, $phone);
                    }
                    break;
            }
        }
    ...
    //var_dump($e);
    ussd_stop("Application under maintenance -1");
}
catch(Exception $e) {
    //var_dump($e);
    ussd_stop("Application under maintenance -2");
}
...
function save($code, $message, $dbh, $phone)
{
    $sql = "INSERT INTO subscribe_product(user_id, user_type, product_id, oper_code, is_auto_extend, channel_id, account_id) VALUES ('$phone', 0, '$code', 'v', 0, 2, 2)";
    $sth = $dbh->prepare($sql);
    $sth->execute();
    if($sth->errorCode() == 0) {
        ussd_stop($message);
